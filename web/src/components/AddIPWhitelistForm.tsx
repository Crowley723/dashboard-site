...
import type { FirewallAlias } from '@/types/Firewall.ts';

interface AddIPWhitelistFormProps {
  className?: string;
...
export function AddIPWhitelistForm({
  className,
  aliases,
  onSubmit,
  isLoading,
  errorMessage,
  successMessage,
}: AddIPWhitelistFormProps) {
  const [aliasName, setAliasName] = React.useState('');
  const [ipAddress, setIPAddress] = React.useState('');
  const [description, setDescription] = React.useState('');
  const [useCustomTTL, setUseCustomTTL] = React.useState(false);
            ...
            <div className="flex flex-col gap-2">
              ...
              {selectedAlias && (
                <span className="text-muted-foreground text-xs">
                  Max {selectedAlias.max_ips_per_user} IPs per user
